local options = {
  ...
  expandtab = true, -- in insert mode: use the appropriate number of spaces to inset a <Tab>
  ...
  inccommand = "split", -- show the ffects of a command incrementally in the buffer (:h "inccommand")
  incsearch = true, -- show where the pattern matches while typing
  hidden = false, -- when off, a buffer is unloaded when it is abandoned (closed)
  ...
  numberwidth = 4, -- number of columns to use for the line number (only if number/relative number is set)
  pumheight = 10, -- maximum number of items to show in popup-menu
  pumblend = 30, -- enables pseudo-transparency for in popup-menu
  ...
  shiftwidth = 2, -- number of spaces to use for each step of (auto)indent, >>, and <<
